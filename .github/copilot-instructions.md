...
This repository parses formula PDFs from the workspace root and writes structured JSON output to `Output/`.

- Runtime: Node.js with TypeScript in ESM mode.
- Entry point: `src/index.ts`.
- Main batch flow: `src/batch/processAllPdfs.ts`.
- Parsing pipeline: PDF read -> text normalization -> field extraction -> JSON write -> MySQL persistence.
...
Run `npm run build` after code changes. If parsing logic changes, also run `npm start` to verify the generated JSON shape.

## Architecture

- `src/index.ts`: CLI entry point that reports batch success and failures.
- `src/batch/processAllPdfs.ts`: finds all `*.pdf` files in the workspace root, processes them sequentially, and writes consolidated output.
...
- `src/io/writeJson.ts`: writes per-file JSON and `Output/formulas.json`.
...
- Keep JSON output generation and database persistence in sync; changes to output shape must be reflected in DB mapping when applicable.
...
`npm start` rewrites JSON files in `Output/`. `npm run build` regenerates `dist/`.
...
- For output shape changes: update `src/types/formula.ts`, then adjust parser and JSON writing logic together.
