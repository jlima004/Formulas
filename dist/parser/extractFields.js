...
function looksLikeSameLineOnlyLabel(line, label) {
    return normalizeForMatch(line) === normalizeForMatch(label);
}
function isLikelyNoise(line) {
    const normalized = normalizeForMatch(line);
    return (normalized.length === 0 || normalized === "page" || normalized === "pagina");
}
function extractFromRegex(lines, pattern) {
...
function extractFormulaAndTotalItems(lines, diagnostics) {
    for (const line of lines) {
...
function extractObservation(lines, diagnostics) {
...
function extractColumnHeaders(lines, data, diagnostics) {
    const headerLine = lines.find((line) => /^Codigo\s+Detalle\s+Partes\s+Costo$/i.test(line));
    if (!headerLine) {
        return;
    }
...
function extractItems(lines) {
    const itemPattern = /^(\d+)\s+([A-Z]{2}\d+)\s+(.+?)\s+(\d{1,3}(?:\.\d{3})*,\d{2})\s+(\d{1,3}(?:\.\d{3})*,\d{2})$/;
    const items = [];
    for (const line of lines) {
...
function normalizeDatabaseNumber(value) {
    if (!value) {
        return null;
    }
...
export function extractFields(lines, options = {}) {
    ...
    if (data.hojaN) {
        const sourceLine = lines.find((line) => /^Hoja\s+N[º°]?:\s*(.+)$/i.test(line));
        if (sourceLine) {
    ...
    const countDiff = data.totalItemsValue !== null
    ...
    if (data.totalItems &&
        ...
        warnings.push({
            ...
            message: "A quantidade de itens extraídos difere do Total Items informado no PDF.",
